    ...
    try:
        if event['RequestType'] == 'Create' or event['RequestType'] == 'Update':
            ...
            if directory_active:
                ...
                # Try to get the actual DN from Active Directory
                try:
                    user_info = ds_data_client.describe_user(
                        DirectoryId=directory_id,
                        SAMAccountName=username
                    )
                    user_dn = user_info['DistinguishedName']
                    print(f"Retrieved actual DN for {username}: {user_dn}")
                except Exception as e:
                    print(f"Warning: Could not retrieve actual DN for {username}: {str(e)}")
                    # Fallback to constructed DN with correct OU structure
                    user_dn = f"CN={username},OU=Users,OU=rabbitmq-ldap,DC=rabbitmq-ldap,DC=tutorial,DC=local"
                    print(f"Using fallback DN: {user_dn}")
